...
void hgeParticleSystem::Update(float fDeltaTime)
{
	...
	if(fAge != -2.0f)
	{
		...
		for(i=0; i<nParticlesCreated; i++)
		{
			...
			par->fSpin = info.fSpinStart+hge->Random_Float(0,info.fSpinEnd)-info.fSpinEnd/2.0f;
			par->fSpinDelta = info.fSpinVar;
...
void hgeParticleSystem::Render()
{
	...
	for(i=0; i<nParticlesAlive; i++)
	{
		...
		info.sprite->RenderEx(par->vecLocation.x*fScale+fTx, par->vecLocation.y*fScale+fTy, par->fSpin, par->fSize*fScale, par->fSize*fScale);
		par++;
	}

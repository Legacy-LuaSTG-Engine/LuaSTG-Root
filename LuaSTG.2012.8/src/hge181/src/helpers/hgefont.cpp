...
void hgeFont::Render(float x, float y, int align, const char *string)
{
	...
	if((align&HGETEXT_HORZMASK)==HGETEXT_RIGHT) fx-=GetStringWidth(string, false);
	if((align&HGETEXT_HORZMASK)==HGETEXT_CENTER) fx-=int(GetStringWidth(string, false)/2.0f);
	...
	if((align&HGETEXT_VERTMASK)==HGETEXT_BOTTOM) y+=int(nilne*fHeight*fScale*fSpacing);
	if((align&HGETEXT_VERTMASK)==HGETEXT_MIDDLE) y+=int(nilne*fHeight*fScale*fSpacing/2.0f);
	...
	while(*string)
	{
		if(*string=='\n')
		{
			...
			fx = x;
			if((align&HGETEXT_HORZMASK) == HGETEXT_RIGHT)  fx -= GetStringWidth(string+1, false);
			if((align&HGETEXT_HORZMASK) == HGETEXT_CENTER) fx -= int(GetStringWidth(string+1, false)/2.0f);
		}
		else
		{

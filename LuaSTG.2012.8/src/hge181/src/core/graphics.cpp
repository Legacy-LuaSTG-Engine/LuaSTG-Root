...
void CALL HGE_Impl::Gfx_SetViewport(int x1, int x2, int y1, int y2)
{
	D3DVIEWPORT8 vp;
...
void CALL HGE_Impl::Gfx_SetOrtho(float x1, float x2, float y1, float y2)
{
	_render_batch();

	D3DXMATRIX tmp;
	D3DXMatrixTranslation(&matProj, -0.0f, -0.0f, 0.0f);
	D3DXMatrixOrthoOffCenterLH(&tmp, x1, x2, y1, y2, 0.0, 1.0);
	D3DXMatrixMultiply(&matProj, &matProj, &tmp);
	pD3DDevice->SetTransform(D3DTS_PROJECTION, &matProj);
}

void CALL HGE_Impl::Gfx_SetPerspective(float eyex,float eyey,float eyez,float atx,float aty,float atz,float upx,float upy,float upz,float fovy,float aspect,float zn,float zf)
{
	_render_batch();

	D3DXMATRIX tmp;
	D3DXMatrixTranslation(&matProj, -0.0f, -0.0f, 0.0f);
	D3DXMatrixLookAtLH(&tmp,&D3DXVECTOR3(eyex,eyey,eyez),&D3DXVECTOR3(atx,aty,atz),&D3DXVECTOR3(upx,upy,upz));
	D3DXMatrixMultiply(&matProj, &matProj, &tmp);
	D3DXMatrixPerspectiveFovLH(&tmp,fovy,aspect,zn,zf);
	D3DXMatrixMultiply(&matProj, &matProj, &tmp);
	pD3DDevice->SetTransform(D3DTS_PROJECTION, &matProj);
}

void CALL HGE_Impl::Gfx_SetFog(float start, float end, DWORD color)
{
	_render_batch();
	if(start!=end)
	{
		...
		if(start==-1.0f)
		{
			...
			pD3DDevice->SetRenderState(D3DRS_FOGDENSITY, *(DWORD *)(&end));
		}
		else if(start==-2.0f)
		{
...
HTTF CALL HGE_Impl::TTF_Load(const char *fontname, LONG width, LONG height, LONG weight, LONG deco, BYTE charset)
{
	LOGFONT lf;
	...
	lf.lfWidth = width;
	lf.lfWeight=weight;
	lf.lfItalic=(deco&0x1);
	lf.lfUnderline=(deco&0x2);
	lf.lfStrikeOut=(deco&0x4);
	lf.lfCharSet=charset;
	lf.lfQuality=ANTIALIASED_QUALITY;
	strcpy(lf.lfFaceName,fontname);
	ID3DXFont * font = 0;
	D3DXCreateFontIndirect(pD3DDevice,&lf,&font);
	fnt.insert(HTTF(font));
	return HTTF(font);
}
void CALL HGE_Impl::TTF_Free(HTTF font)
{
	fnt.erase(HTTF(font));
	((ID3DXFont *)font)->Release();
}
void CALL HGE_Impl::TTF_Draw(HTTF font,const char *str, LONG l,LONG t,LONG r,LONG b,DWORD fmt,DWORD color)
{
	_render_batch();
	RECT rect={l,t,r,b};
	((ID3DXFont *)font)->DrawText(str,-1,&rect,fmt,color);
}
...
bool HGE_Impl::_GfxRestore()
{
	...
	for(std::set<HTTF>::iterator it=fnt.begin();it!=fnt.end();it++)
		((ID3DXFont *)(*it))->OnLostDevice();

	pD3DDevice->Reset(d3dpp);

	for(std::set<HTTF>::iterator it=fnt.begin();it!=fnt.end();it++)
		((ID3DXFont *)(*it))->OnResetDevice();

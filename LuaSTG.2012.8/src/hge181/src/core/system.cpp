...
bool CALL HGE_Impl::System_Start()
{
	...
	for(;;)
	{
		...
		if(bActive || bDontSuspend)
		{
			...
			if((fFPSCorAcc < 1.0f && dt >= nFixedDelta) || (fFPSCorAcc >= 1.0f && dt >= nFixedDelta+1))
			{
				//FPS correction

				if(fFPSCorAcc >= 1.0f) fFPSCorAcc-=1.0f;
				fFPSCorAcc+=fFPSCor;
...
void CALL HGE_Impl::System_SetStateBool(hgeBoolState state, bool value)
{
	...
	{		
		...
		case HGE_VSYNC:			if(VertArray) break;
								if(pD3DDevice)
								{
									if(value)
									{
	...
	switch(state)
	{
		...
		case HGE_FPS:			if(VertArray) break;
								...
								if(nHGEFPS>0)
								{
									...
									fFPSCor=1000.0f/value-nFixedDelta;
								}
								else nFixedDelta=0;
								break;
	}

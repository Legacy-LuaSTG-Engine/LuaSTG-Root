...
enum hgeBoolState
{
	...
	HGE_VSYNC			= 8,	// bool		vsync?				(default: ?)
...
public:
	...
	virtual void		CALL	Gfx_SetViewport(int x1, int x2, int y1, int y2) = 0;
	virtual void		CALL	Gfx_SetTransform(float x=0, float y=0, float dx=0, float dy=0, float rot=0, float hscale=0, float vscale=0) = 0; 
	virtual void		CALL	Gfx_SetOrtho(float x1, float x2, float y1, float y2) = 0;
	virtual void		CALL	Gfx_SetPerspective(float eyex,float eyey,float eyez,float atx,float aty,float atz,float upx,float upy,float upz,float fovy,float aspect,float zn,float zf) = 0;
	virtual void		CALL	Gfx_SetFog(float start, float end, DWORD color) = 0;
	...
	virtual HTTF		CALL	TTF_Load(const char *fontname, LONG width, LONG height, LONG weight, LONG deco, BYTE charset) = 0;
	virtual void		CALL	TTF_Free(HTTF font) = 0;
	virtual void		CALL	TTF_Draw(HTTF font, const char *str, LONG l,LONG t,LONG r,LONG b,DWORD fmt,DWORD color) = 0;
};
